...
public class AccountTransactionModel extends AbstractModel
{
    ...
    {
        return type == Type.DIVIDENDS && security != null ? security.getWithholdingTaxRate() : null;
    }

    /**
     * Applies the withholding tax rate configured on the security. For foreign
     * currency dividends, taxes are entered in the security currency. Any tax
     * amount in the other currency is cleared so the tax is never counted
     * twice.
     */
    public void applyWithholdingTaxRate()
    {
        BigDecimal rate = getWithholdingTaxRate();
        if (rate == null)
            return;

        boolean isForex = !getAccountCurrencyCode().isEmpty() && !getSecurityCurrencyCode().isEmpty()
                        && !getAccountCurrencyCode().equals(getSecurityCurrencyCode());

        long newFxTaxes = isForex ? calculateTaxes(fxGrossAmount, rate) : 0;
        long newTaxes = isForex ? 0 : calculateTaxes(grossAmount, rate);

        firePropertyChange(Properties.fxTaxes.name(), this.fxTaxes, this.fxTaxes = newFxTaxes); // NOSONAR
    ...
    private long calculateTaxes(long grossValue, BigDecimal rate)
    {
        return BigDecimal.valueOf(grossValue).multiply(rate).setScale(0, RoundingMode.HALF_UP).longValue();
    }
